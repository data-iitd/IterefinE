...
test(model,valid_loader,valid_features,arg_threshold=best_threshold,dname=dataset_lin[-1]+"_ConvE_valid_preds_"+args.early_pred+".txt",early_pred=args.early_pred)
print('Evaluation on test set with best model saved based on valid set')
test(model,test_loader,test_features,arg_threshold=best_threshold,dname=dataset_lin[-1]+"_ConvE_test_preds_"+args.early_pred+".txt",early_pred=args.early_pred)
