...
def createModel(data, settings){
    ...
    m.add rule: ( CandRel_Neural(A,B,R) ) >> Rel(A,B,R),
    squared: sqPotentials,
    weight : weightMap["CandRel_Neural"];
...
def runInference(m, data, settings){
    ...
    Partition candidates = new Partition(55);
    ...
    HashSet closedPredsAll = new HashSet<StandardPredicate>([Name,Sub,RSub,Mut,RMut,Inv,Domain,Range2,SameEntity,CandCat,CandRel,TrCat,TrRel,CandRel_Neural]);

    System.out.println("STATUS: Starting Inference with Testing Targets and Learned Model");
    Date testingInference = new Date();

    Database inferenceTesting = data.getDatabase(writeInfTe, closedPredsAll, ontology, entity_resolution, training, candidates);
