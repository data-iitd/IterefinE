...
do 
	...
	mkdir wn18rr_ConvE_$episode
	mv out wn18rr_ConvE_$episode/out
	rm -rf out
	mkdir NN_data
	...
	echo "training NN"
	cd type_complex
	python3 main.py -d ../NN_data -m typed_model -a '{"embedding_dim":19, "base_model_name":"ConvE", "base_model_arguments":{"embedding_dim":180}}' -l softmax_loss -e 20 -r 0.5 -g 0.3 -b 128 -x 50 -n 50 -v 1 -q 0	
	free -g
	free -k
	free -m
	nvidia-smi
	...
	python3 main.py -d ../NN_data -m typed_model -a '{"embedding_dim":19, "base_model_name":"ConvE", "base_model_arguments":{"embedding_dim":180}}' -l softmax_loss -e 20 -r 0.5 -g 0.3 -b 128 -x 50 -n 50 -v 1 -q 0         --dump_from model/model.pt
	cd ..
	...
	mv -f *.txt wn18rr_ConvE_$episode/
	mv -f psl.* wn18rr_ConvE_$episode/
	mv -f NN_data/ wn18rr_ConvE_$episode/NN-data/
	mkdir wn18rr_ConvE_$episode/type_complex
	mkdir wn18rr_ConvE_$episode/type_complex/model
	mv -f type_complex/*.txt wn18rr_ConvE_$episode/type_complex/
	mv -f type_complex/model/model.pt wn18rr_ConvE_$episode/type_complex/model/model.pt
	rm -rf *.txt
	rm -rf psl.* 
	rm -rf NN_data
