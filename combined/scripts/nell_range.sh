...
do 
	...
	mkdir nell_range_$episode
	mv out nell_range_$episode/out
	rm -rf out
	mkdir NN_data
	...
	mv -f *.txt nell_range_$episode/
	mv -f NN_data/ nell_range_$episode/NN-data/
	mkdir nell_range_$episode/type_complex
	mkdir nell_range_$episode/type_complex/model
	mv -f type_complex/*.txt nell_range_$episode/type_complex/
	mv -f type_complex/model/model.pt nell_range_$episode/type_complex/model/model.pt
	rm -rf *.txt
	rm -rf psl.* 
	rm -rf NN_data
...
do 
	...
	stdbuf -oL java -Xmx60800m -cp ./target/classes/edu/umd/cs/psl/kgi/:./target/classes:`cat classpath.out` edu.umd.cs.psl.kgi.RunKGI_nell_org &> out # run PSL-KGI
	python3 gen_predicts.py out cat_predicts.txt rel_predicts.txt
	mkdir nell_inv_$episode
	mv out nell_inv_$episode/out
	rm -rf out
	mkdir NN_data
	python3 evaluate_and_update.py rel_predicts.txt data/test_relations.txt cat_predicts.txt data/test_labels.txt data/dev_relations.txt data/dev_labels.txt
	python3 generate_types.py cat_predicts.txt data/names.txt data/165.onto-wbpg.db.Sub.txt data/165.onto-wbpg.db.Domain.txt data/165.onto-wbpg.db.Range2.txt
	...
	python3 convert_neural_src.py NN_data/valid.txt type_complex/train_preds.txt data/names.txt data/Neural.Rel.out 
	python3 print_scores.py 
	mv -f *.txt nell_inv_$episode/
	mv -f NN_data/ nell_inv_$episode/NN-data/
	mkdir nell_inv_$episode/type_complex
	mkdir nell_inv_$episode/type_complex/model
	mv -f type_complex/*.txt nell_inv_$episode/type_complex/
	mv -f type_complex/model/model.pt nell_inv_$episode/type_complex/model/model.pt
	rm -rf *.txt
	rm -rf psl.* 
	rm -rf NN_data
	rm -f type_complex/*.txt
