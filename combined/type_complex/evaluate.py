...
class ranker(object):
    ...
    def forward(self, s, r, o, r_d,r_r,knowns, flag_s_o=0):
        """
        ...
        """
        if flag_s_o:
            scores = self.scoring_function(s, r, None,r_d,r_r).data
            score_of_expected = scores.gather(1, o.data)
        else:
            scores = self.scoring_function(None, r, o,r_d,r_r).data
...
def my_evaluate(name,dname,ranker,kb,batch_size,verbose=0,top_count=5,hooks=None,labels = [],eval_cnt = 0, threshold=None):
    facts = kb.facts
    scores = []
    for i in range(0,int(facts.shape[0]),batch_size):
        ...
        r = facts[start:end, 1]
        o = facts[start:end, 2]
        r_d = facts[start:end,3]
        r_r = facts[start:end,4]
        t_s = facts[start:end,5]
        t_o = facts[start:end,6]
        ...
        r_d = torch.autograd.Variable(torch.from_numpy(r_d).cuda().unsqueeze(1), requires_grad=False)
        r_r = torch.autograd.Variable(torch.from_numpy(r_r).cuda().unsqueeze(1), requires_grad=False)
        t_s = torch.autograd.Variable(torch.from_numpy(t_s).cuda().unsqueeze(1), requires_grad=False)
        t_o = torch.autograd.Variable(torch.from_numpy(t_o).cuda().unsqueeze(1), requires_grad=False)
        batch_scores = ranker.forward(s,r,o,r_d,r_r,t_s,t_o).data.tolist()
        #print('batch',batch_scores)
        scores.extend([i[0] for i in batch_scores])
    #print('my evaluation',name)
    #print(labels[:10],scores[:10])
    #print(name,eval_cnt,'auc',average_precision_score(labels,scores))
    thresholds = {i:0 for i in [j/100 for j in range(5,100,5)]}
    if dname:
        if threshold: scores=[(1 if i>=threshold else 0) for i in scores]
        with open(dname,'w') as f:
            for sc in scores: f.write('%f\n'%(sc))
    if labels==[] : return None
    best_sc = 0.0
    best_threshold = 0.0
    for threshold in thresholds:
        predictions = [(1 if i>=threshold else 0 ) for i in scores]
        thresholds[threshold] = f1_score(labels,predictions,average='weighted')
        if thresholds[threshold]>=best_sc:
            best_sc = thresholds[threshold]
            best_threshold = threshold
    #print(name,eval_cnt,'f1 scores', thresholds)

    return best_threshold,best_sc
