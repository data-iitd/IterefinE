...
def main(dataset_root, save_dir, model_name, model_arguments, loss_function, learning_rate, batch_size,
         ...
         eval_every_x_mini_batches, eval_batch_size, resume_from_save, introduce_oov, verbose,dwrite,dump_from):
    ktrain = kb.kb(os.path.join(dataset_root, 'train.txt'))
    types,domains,ranges,label_size = ktrain.build_type_info(dataset_root)
    if introduce_oov:
    ...
    ktest = kb.kb(os.path.join(dataset_root, 'test.txt'), ktrain.entity_map, ktrain.relation_map,
                  add_unknowns=not introduce_oov,domains = domains,ranges= ranges,types=types)
    ...
    model_arguments['label_count'] = label_size
    model_arguments['types'] = types
    model_arguments['domains'] = domains
    model_arguments['ranges'] = ranges
    ...
    valid_labels = [int(i.rstrip()) for i in open(dataset_root+'/valid_labels.txt')]
    test_labels = [int(i.rstrip()) for i in open(dataset_root+'/test_labels.txt')]
    if dump_from:
        tr.dump(dump_from,valid_labels=valid_labels,test_labels=test_labels)
    else:
        tr.start(max_mini_batch_count, [eval_every_x_mini_batches//20, 20], mb_start,valid_labels = valid_labels,test_labels = test_labels,dname=dwrite)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w', '--write',required=False,default='')
    ...
    parser.add_argument('--dump_from',default=None,required=False)
    ...
    main(dataset_root, arguments.save_dir, arguments.model, arguments.model_arguments, arguments.loss,
         ...
         arguments.oov_entity, arguments.verbose,arguments.write,arguments.dump_from)
